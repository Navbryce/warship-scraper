...
from ABoatScraping.Armament import Armament
from ABoatScraping.Armor import Armor
from ABoatScraping.Date import Date
...
from ABoatScraping.ship_compare.ship_network_algo.run_algos_on_network import run_algos
from ABoatScraping.utilities.get_environment import CONFIG_PATH, RECOMMENDED_UNITS
from ABoatScraping.utilities.text_functions import get_text_paragraph
...
def getWordsBeforeUnit(haystack, unit, numberOfWords):
    ...
    while positionOfUnit != -1 and positionOfUnit + unitLength != len(haystack) and haystack[
        positionOfUnit + unitLength] != ' ':  # makes sure the unit is by itself if not at the end of the string
        ...
        while len(words) < numberOfWords and position != -1:
            ...
            if character == ' ':
                if position + 1 != mostRecentSpace:  # makes sure it isn't just a space before a space
                    word = haystack[
                           position + 1:]  # The indexes after the space to the end of the string should all be words
                    words.append(word)
...
def createStringFromArray(startingIndex,
                          arrayOfWords):  # creates string from array of words. first word is at starting index
...
def getArrayOfWords(haystack, needle,
                    numberOfWords):  # number of words including needle. if needle is None, then start at beginning of haystack. if numberOfWords==-1, then all words
    words = []
    if needle:
...
def getImages(pageElement,
              maxImages):  # Page element = DOM element that is a parent (direct or indirect) of the pictures
        ...
        for imageWrapper in imagesWrappers:  # Image wrapper is the container for the caption and image
            ...
            if len(images) > 0:
                ...
                src = image.attrib["src"][
                      2:]  # remove the first two characters of the URL because they are a filepaths for the server ('//'), not URL. Represent root.
...
def formatShipName(shipName):
    ...
    wordsToFilter = ["hms", "uss", "battleship", "german", "french", "japanese", "italian", "american", "ijn", "(",
                     "sms", "cruiser"]  # in lower case. shipName will be converted to lower case for comparison
...
def processArmament(armamentElement, armamentString, arrayOfWords,
                    armamentTypeObjects):  # Should probably transfer ALL of this code to the Armament class
    """
    ...
    quantity = conversionTable.convertUnit(quantityBeforeConversion, "word", arrayOfName[
        1])  # If double, triple are the first words, will multiply the quantity by the appropriate multiplier. If it's some other word (or number), it will just return the original value
    ...
    if quantity is None:  # Should only be called if an error occurs. For example: New York and torpedo tubes
        ...
        charactersToRemove = ['(', ')']
        armamentFullTextWithout = removeOddCharacters(armamentString,
                                                      charactersToRemove)  # not necessarily the full name
        characterToSpace = ['/']
        ...
        armament[
            'pictures'] = pictures  # Should really be set in the armament constructor. Pictures defined at the top of the function and assigned under certain conditions in the link if statement
...
def processArmor(armorElement, listTitleInput):
    ...
    armorFormattedText = formatString(replaceColonWithSpace((
        armorElement.text_content())))  # replace colon with space because some pages have errors where there is no space after the colon
    ...
    if len(arrayOfWords) >= 1:
        ...
        if (
                len(arrayOfWords) >= 2):  # FILTERS OUT: We are dealing with the title to a list of armor elements below this one
            oddCharacters = ["(", ")"]
            armorFullName = removeOddCharacters(armorFormattedText, oddCharacters)

            units = ["mm"]
            for unit in units:
                armorWidthArray = getWordsBeforeUnit(armorFullName, unit,
                                                     3)  # Get three because a possible situation: {min} to {max}
                if len(armorWidthArray) > 1:
                    armorUnit = unit
                    if len(armorWidthArray) == 3 and armorWidthArray[
                        1] == "to":  # If there aren't three words, that means it's impossible for it to be a range. @ Index 1 is where "to" should be
                        armorRange = True
                        ...
                        armorRange = True
                        armorRangeValues = armorWidthArray[
                            0]  # The armor is a range. The first word before the unit has a dash in it (presumbably A-B where A and B are some constant)
                    ...
                    if listTitleInput != None and len(
                            armorElement.cssselect("a")) == 0:  # you can identify a sub list item by its LACK of a link
...
def processStandardValue(valueString):
    ...
    unitsToSearchFor = ["km", "nmi", "miles", "kn", "knots", "m", "meters", "ft", "feet", "t", "long tons", "tons",
                        "kw", "in",
                        "inch"]  # Range units prioritized first because some ranges have speeds associated with them (we don't want to pull the speed value)

    searchString = removeOddCharacters(valueString,
                                       oddCharacters)  # Removes parentheses because some units are surrounded by them
    searchString = replaceOddCharacters(searchString, replaceWithSpaceCharacters, ' ')
    for unit in unitsToSearchFor:
        wordsBeforeUnit = getWordsBeforeUnit(searchString, unit,
                                             1)  # Returns an array of words before the unit of at most size 1. Note: units must be surrounded by spaces
...
def calculateComplement(valueString, ship):
    ...
    valueStringContainsOfficersAndEnlisted = valueString.find("officers") > 0 and valueString.find(
        "enlisted") > 0  # IF true, the any other "complement" strings must just be from a different configuration, so only keep the value from the latest complement string(don't sum all values).
...
def processArmamentElements(armamentElements, configurationToKeep):
    ...
    for arrayValueElement in armamentElements:
        ...
        if isDate:
            ...
            if configurationCounter > configurationToKeep or len(
                    values) > 0:  # Stop adding armaments if you've encountered another date past your configuration OR you've added armaments and have encountered a date
            ...
            if armamentFormattedString.find(
                    "removed") == -1:  # Some times armaments are listed when they are removed. Ensures the armament wasn't removed
                armament = processArmament(arrayValueElement, armamentFormattedString, arrayOfWords,
                                           armamentTypeObjects)
    ...
    for armamentType in typesOfArmament:
        ...
        armamentTypeObject["sizeCalculate"] = armamentTypeObject[
            "sizeCalculate"].calculationsDictionary()  # Serializes the calculate object for size
        armamentTypeObject["armaments"] = armamentTypeObject["armaments"]

    return armamentTypeObjects


def categorizeElement(key, value, valueElement,
                      ship):  # Will categorize elements that are not already in "arrays." For example, commissioned, decomissioned, recomissioned, ... are all listed as separate elements in the highest level of table
    ...
    physicalAttributeKeyWord = ['draught', 'displacement', 'length', 'beam', 'depth of hold', 'tons burthen', 'speed',
                                'draft', 'range', 'installed power']
    ...
    if physicalAttribute:
        ...
        if date is not None:
            ...
            'awards') >= 0:  # Some pages have this as a list. Some only have it as a single element. Standardizes to an array
        ship['awards'].append(value)
    elif key.find(
            'class') >= 0:  # Separates class and type in a cell element. First word is always class. Words after are always type. Some ships only have a "type" not a class. This statement won't evluate in that situation, so it still works.
...
def processRow(rowElement, shipBeingUpdated):
    ...
    if len(cellElements) == 2 and len(cellElements[1].cssselect(
            'img')) == 0:  # Ensure it actually has a value and the value does not have a nimage
        ...
        if len(arrayValueElements) == 0:  # dealing with a single value element aka not an array
            categorizeElement(key, formatString(valueElement.text_content()), valueElement,
                              ship)  # the object referenced by 'ship' modified in function. thus, no ship returned because reference points to the same object
        else:
            keysWithNoArray = ["draught", "length", "displacement", "speed",
                               "draft", "propulsion",
                               "installed power"]  # Should contain the keys that should NOT have an array of objects. For example, some ships have multiple displacements listed--scraper should only keep one

            values = []
            addvaluesToShip = True
            if key == "armament":
                configuration = int(shipBeingUpdated[
                                        'configuration'])  # Which configuration do you want if their are multiple configurations
                values = processArmamentElements(arrayValueElements,
                                                 configuration)  # deals with processing armament elements
            elif key == "armor" or key == "armour":
                ...
                for complementElement in arrayValueElements:
                    ...
                    categorizeElement(key, valueString, complementElement,
                                      ship)  # See how categorize elemtn categorizes complement. It will sum the values of all complement elements
                ...
                for noArrayKey in keysWithNoArray:  # keysWithNoArray defined at top
                    if key == noArrayKey:
                        lastValueElement = arrayValueElements[
                            len(arrayValueElements) - 1]  # Will only save the last value
            ...
            if key.lower().find(
                    "awards") >= 0:  # Sometimes awards are listed as a list. Sometimes they are just a single value. categorizeElement makes the awards key "awards". This converts "honors and awards" to "awards" as the key
                key = "awards"
...
try:
    if len(sys.argv) == 1:  # the script name is always the first argument
        ...
        if len(sys.argv) == 2:  # Use default settings because JSON is being directly passed. Not JSONfile path. See help message at bottom
            ...
            shipsInput = json.loads(
                shipsJSON)  # windows shell removes quotes before sending argument, so sending JSON through windows does not work.
        else:
    ...
    print("Please pass: {settingsPath} {ingestPath} or {ingestJSON}")
    print(
        exception)  # Print exception because when running child process from node, only output directly printed to console can be seen
    traceback.print_exc()
    runScript = False
...
if runScript:  # runScript is set false if one of the parameters is bad
    ...
    for page in shipPages:
        ...
        ship = {
            'scrapeURL': page['url'],
            # used to check uniqueness of ship AKA has it already been added. Note: I could check name and date (some ships have the same name), but I only need to do 1 comparison with URL
            'configuration': str(page['configuration']),
